...
export async function lendingListInsert(
    address: string,
    amount: number,
    currency: string,
    duration: number,
    ...
    const { data, error } = await supabase.rpc('insert_lending_list', {
        address: address,
        amount: amount,
        currency: currency,
    ...
    if (error) {
        console.error('Error insert to lending list:', error.message);
        return false;
    }
